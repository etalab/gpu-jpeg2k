...
static float *load_image(const char *image_filename, type_image_hyper *image_bip)
{
	...
	h_imagen = (float*) my_malloc(image_bip->num_lines * image_bip->num_samples * image_bip->num_bands * sizeof(float));
	...
	if ((fp = fopen(image_filename, "rb")) == NULL) {
		...
		break;
		case 2: {
			type_short_int = (short *) my_malloc(image_bip->num_lines * image_bip->num_samples * image_bip->num_bands
					* sizeof(short));
		...
		case 4: {
			type_float = (float *) my_malloc(image_bip->num_lines * image_bip->num_samples * image_bip->num_bands
					* sizeof(float));
		...
		case 5: {
			type_double = (double *) my_malloc(image_bip->num_lines * image_bip->num_samples * image_bip->num_bands
					* sizeof(double));
